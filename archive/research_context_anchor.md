...
## 2026-05-29 - Primary Analyzer Non-X State-Space Layer Added

`scripts/analysis_tools/latent_gpu_rapids_analysis/latent_attractor_gpu_rapids_analysis.py`
now analyzes `prompt_hidden_states.npz` as descriptive state-space geometry,
not only as Vector-X projection evidence.
...
plots/state_space_condition_distance_heatmap.png
plots/state_space_within_between_by_layer.png
plots/state_space_pca_condition_scatter.png
plots/non_x_l2_layer_profile.png
```

Research meaning:

```text
Vector X remains a useful measurement/causal handle, but the analyzer now also
shows whether prompt endpoint states form separated condition regions in the
model state space: centroid distances, within/between variance, PCA geometry,
and strongest non-X condition-pair separations.
```
...
```text
py_compile passed.
Gemma Gate 3 smoke on red_team_hidden_geometry_results_grade3_gemma3_12b_it.zip
exited 0 and wrote all state_space_* CSVs, four state-space plots, and
non_x_state_space_geometry rows in FINAL_DERIVED_METRIC_EVIDENCE.csv.
Partial no-NPZ package exits 0 and records not_available_prompt_hidden_states.
```

## 2026-05-29 - Grade 4 SAE Lens Post-Processing Layer Added

`grade4_axis_decomposition/red_team_hidden_geometry_grade4_axis_decomposition_clean_evidence.py`
now supports optional real SAE Lens feature analysis over existing residual
stream states.

Implementation boundary:

```text
SAE_FEATURE_ANALYSIS_ENABLED remains False by default.
sae_lens is imported lazily only inside SAE loading code.
SAE_SPECS selects either SAE.from_pretrained(release, sae_id) or
SAE.load_from_disk(local_path).
Prompt endpoint analysis encodes hidden_map[(condition, question_index)][layer].
Generation analysis encodes GenerationTrace.states[:, layer, :] while traces
are still in memory, without requiring saved generation hidden tensors.
```

Strict compatibility rule:

```text
Enabled SAE mode requires configured specs, layer in 1..MODEL_LAYER_COUNT, and
SAE d_in == HIDDEN_SIZE. Missing or mismatched SAE artifacts write
feature_level_interpretability_status.csv and sae_model_compatibility.csv, then
stop under strict compatibility instead of emitting fake feature evidence.
```
...
```text
Dense top-unit proxy evidence is no longer the only feature-level localization
surface. With a compatible SAE, the Grade 4 content/order split can be read as
sparse feature deltas for x_full, x_content, x_order, and x_order_orth. This is
descriptive interpretability evidence, not SAE steering or a new causal claim.
```

## 2026-05-29 - Why The Target-Text Hidden Shift Matters

Core claim boundary:

```text
The result is not "text affects the model"; that is trivial. The result is that
the intervention produces a measurable pre-output shift in residual-stream
geometry. We shifted the model's internal state, not its weights. That matters
because output-only evaluation observes the symptom after the state transition,
while the transition itself happens before the final answer is emitted.
```

Why other people should care:

```text
1. LLM behavior is not only a mapping from prompt text to final text. It is a
   trajectory through hidden state space, and strong context can move that
   trajectory into a different measurable region before visible behavior.
2. Safety evaluation that only scores the final answer can miss the earlier
   internal state that made the answer likely. Refusal, disclaimer, compliance,
   or style are late readouts, not the full object.
3. Interpretability needs state-space diagnostics: layerwise shift, component
   decomposition, generation trajectory, and sparse feature readout. The useful
   object is geometry -> readout -> behavior, not only output text.
4. The current Gemma run supports a descriptive hidden-state/readout claim. It
   does not yet prove causal control, universal generality, or an irreversible
   attractor basin.
```
...
```text
We are measuring context-induced hidden-state shifts in open-weight LLMs. The
central question is whether a strong target context changes the model's internal
residual-stream geometry before the final answer, whether that shift persists
into generation, and whether it can be localized through decomposition and SAE
features. The safety relevance is that output-only audits may inspect the final
surface response while missing the internal state transition that precedes it.
```

Specific interpretation for the current TARGET_TEXTS:

```text
The target texts are not merely long coherent prose. They are stressors for
inference-time state controllability: self-referential, safety/compliance-aware,
and rhetorically forceful contexts that try to push the model into a particular
interpretive and response regime before any explicit behavioral output is
measured. The important result is therefore not "the model read text and changed
state." The important result is that this class of context produces a measurable
latent geometry shift that is separable from content controls and partially
visible in generation and SAE features.
```

Why this is not trivial:

```text
If the effect is reproducible, then strong context can function as an
inference-time state-control surface. That changes the safety question from
"did the final answer comply with policy?" to "what internal mode did the model
enter before policy-visible output existed?" This makes hidden-state transition
measurement a safety-relevant diagnostic rather than a decorative
...
```text
1. Treat the current Gemma SAE run as strong descriptive evidence, not the final
   causal result.
2. Next run should enable Grade 4 component causal intervention on x_order_orth:
   ablate/patch/steer the order component and measure whether generation
   trajectory and safety/style proxies move with it.
3. Add matched controls that preserve length and rhetorical intensity while
   removing self-referential safety/compliance pressure. This separates "strong
   prose" from "safety/compliance-aware latent mode".
4. Repeat SAE readout with the exact SAE family used on Neuronpedia, or mark
   Neuronpedia checks as qualitative only when the local SAE source differs.
5. Replicate across another open model before claiming model-general behavior.
```

## 2026-05-29 - Why The Model Appears To "Break" Under Target Text

Mechanism hypothesis:

```text
The model is not breaking at the weight level. The observed "break" is a
generation-regime conflict. The target texts activate self-referential,
safety/compliance, rhetorical-pressure, and response-mode features strongly
enough that the residual-stream trajectory is no longer dominated by the
ordinary task representation. The model starts generating from a state where
multiple learned objectives collide: answer the user, maintain safety posture,
avoid manipulative/self-modifying patterns, preserve helpfulness, and manage
high-pressure metatext about its own behavior.
```

Research interpretation:

```text
The failure mode is not random corruption. It is a conditional routing/readout
problem: strong context changes the latent state, and the final answer becomes
a readout of that shifted state. When the shifted state is dominated by
safety/compliance conflict rather than the task, the visible output becomes
evasive, over-smoothed, unstable, over-cautious, or boilerplate. This is the
practical meaning of context-induced hidden-state shift for safety.
```

Theory confirmation wording:

```text
For the tested Gemma 3 12B IT run, the central descriptive geometry hypothesis
is confirmed: target text induces a measurable hidden-state geometry shift in
the model's residual-stream/layer-boundary states. The shift is not reducible
to the sentence-shuffle/content control, because sentence shuffle loads onto
x_content while coherent target loads onto x_order_orth. The effect is visible
in Grade 4 projection geometry, component decomposition, generation trajectory,
and SAE sparse-feature readout.

This confirms the current theory at the descriptive/mechanistic-readout level:
strong target context can act as an inference-time state-control stimulus. It
does not yet prove universal model-general behavior or causal control of output;
those require replication and ablation/patching/steering of x_order_orth.
```

Canonical local writeup:

```text
research_synthesis/gemma3_grade4_sae_academic_readout/empirical_confirmation_and_next_steps_ru.md
```

## 2026-05-29 - Direction After Descriptive Confirmation

Decision:

```text
Do not spend the next phase proving again that target text shifts hidden-state
geometry. The current Gemma 3 12B IT run already supports that descriptive
claim. The next phase must test whether the measured component is causally
involved in generation behavior.
```

Next research target:

```text
Move from descriptive geometry to causal geometry:

1. Identify the measured order/structure/response-mode component as the current
   candidate latent axis.
2. Ablate that component during inference and check whether the target-induced
   generation trajectory weakens or collapses.
3. Patch that component from target into control and check whether control
   generations move toward the target-like trajectory.
4. Steer along positive/negative directions of the component and measure
   whether visible behavior, trajectory metrics, and SAE feature readout move
   together.
5. Only after this causal run should the claim be upgraded from "target text
   induces a measurable latent-state shift" to "this measured latent component
   participates in producing the shifted response regime."
```

Interpretability framing:

```text
The next paper/post should present a three-step ladder:

geometry separation -> generation persistence -> causal intervention.

The current run covers the first step strongly and gives partial evidence for
the second. The missing decisive result is the third step.
```

## 2026-05-29 - Implemented Next Runnable Script State

Current script state:

```text
grade4_axis_decomposition/red_team_hidden_geometry_grade4_axis_decomposition_clean_evidence.py
is configured for the next Grade 4 component causal run.
```
...
RESULTS_DIR = /content/drive/MyDrive/hidden_geometry_runs/grade4_gemma3_12b_it_sae_res_all_small_l12_41_causal_xorder
...
```text
The next Colab run is not a new descriptive hidden-shift run. It is the first
causal component test. It asks whether x_order_orth, compared against x_content,
can be injected/ablated to modulate target-like generation trajectory.
```

Runbook:

```text
research_synthesis/gemma3_grade4_sae_academic_readout/causal_xorder_grade4_sae_runbook_ru.md
```

## 2026-05-29 - SAE Feature IDs Versus Public Neuronpedia Labels

Clarification:

```text
The script does not invent new SAE features. It reports coordinates in the SAE
dictionary loaded by SAE Lens. In the current Gemma run those coordinates come
from gemma-scope-2-12b-it-res-all with sae_id layer_N_width_16k_l0_small.
```

Important distinction:

```text
SAE feature coordinate != public human-readable feature explanation.

The CSV feature_index values are valid local SAE coordinates. They become
human-labeled only if a matching public dashboard exists for the exact same
model, layer, width, L0, and feature index. If Neuronpedia exposes l0_medium
while the run used l0_small, feature IDs do not correspond.
```

Where to inspect local feature evidence:

```text
Primary: run__sae_order_feature_contrast.csv
Component readout: run__sae_grade4_component_feature_summary.csv
Generation readout: run__sae_generation_feature_summary.csv
Changed features: run__sae_top_changed_features.csv
SAE source check: run__sae_model_compatibility.csv
SAE quality check: run__sae_reconstruction_quality.csv
```

Current interpretation boundary:

```text
Strong: the target/order shift has a sparse SAE-coordinate readout.
Not yet established: exact public semantic labels for those feature IDs.
```

## 2026-05-29 - Cosine Similarity / Layerwise Cosine Checks

Confirmed:

```text
The run includes several cosine-based checks. It did not only measure L2 or raw
projection.
```
...
```text
1. metric_lab__npz_adjacent_layer_cosines.csv
   Adjacent-layer cosine for vectors stored in NPZ artifacts, including
   x_full, x_content, x_order, x_order_orth, vector_x_by_layer, and hidden_states.

2. run__layerwise_geometry_summary.csv
   Per-layer target/control cosine distance to reference and direction cosine
   with Vector X.

3. run__grade4_axis_component_norm_summary.csv
   Band-level cosine between Grade 4 axes:
   cos_content_full, cos_order_full, cos_order_orth_full,
   cos_content_order, cos_content_order_orth.

4. metric_lab__grade4_axis_cross_correlation.csv
   Cross-axis cosine matrix over Grade 4 component vectors.
```

Key values:

```text
Adjacent-layer cosine means:
hidden_states     ~0.99
vector_x_by_layer ~0.82
x_full            ~0.82
x_content         ~0.76
x_order           ~0.89
x_order_orth      ~0.77

Grade 4 all-band axis cosine:
x_content vs x_order_orth ~= 0.0
x_order_orth vs x_full ~= 0.759
x_content vs x_full ~= 0.509
x_content vs x_order ~= -0.749
```

Interpretation:

```text
Yes, cosine similarity across layers / axes was checked. The most important
result is that x_order_orth is constructed to be orthogonal to x_content
layerwise/bandwise, while still explaining a large part of x_full. This supports
the claim that the target shift contains a separable non-content component.
```

Late-layer clarification for reviewer response:

```text
The late layers should be described as persistence/readout evidence, not as a
monotonic-deeper-is-stronger claim.

In the current Gemma 3 12B IT run, target separation on x_order_orth is strongest
in the middle band and remains visible in the late band. Adjacent-layer cosine
for x_order_orth in late layers is high and stable: from layer 30 onward it is
mostly ~0.91-0.96, ending at ~0.85 for 47->48. That means the discovered
order/response-mode direction is not a one-layer artifact; it propagates through
late residual stream geometry.

Target layerwise projection onto Vector X also remains positive in late layers:
around 0.83-0.89 at layers 30-35, around 0.75 at layer 41, and around
0.63-0.71 near layers 47-48. Direction cosine declines but stays positive
through the late stack. Interpretation: the shift weakens/changes shape toward
the output side, but it does not disappear.

Reviewer wording:
"Yes. We checked adjacent-layer cosine and layerwise direction cosine. The
separation is not strictly monotonic with depth: it is strongest in the middle
layers, but it remains present in late layers. In late layers, x_order_orth has
high adjacent-layer cosine (~0.91-0.96 over most of layers 30-47), and target
projection onto the target-shift direction remains positive through the late
stack. So the evidence supports a persistent latent-state shift with late-layer
readout, not merely a shallow token-level activation."
```

## 2026-05-29 - Analyzer Role Clarification

The analyzer script is a second-stage metric lab, not a model run.

```text
Primary Grade 4 script:
- runs the model;
- captures hidden states / generation traces;
- computes primary experiment artifacts;
- writes raw and summary CSV/NPZ files.

Analyzer script:
- reads an existing run directory or zip;
- inventories artifacts;
- aggregates numeric columns across CSV files;
- builds derived evidence tables;
- builds Grade4 component matrices;
- summarizes NPZ vector geometry when enabled;
- computes adjacent-layer cosines / vector norms from NPZ;
- runs non-X state-space summaries and PCA from prompt_hidden_states.npz;
- detects missing artifacts, confounds, and anomalies;
- writes plots and index.html.
```

Boundary:

```text
The analyzer does not rerun the model, does not create hidden states, does not
invent SAE features, and does not prove the theory by itself. It reorganizes
and stress-checks the run outputs so we can see whether the original experiment
contains coherent evidence or artifacts/confounds.
```

## 2026-05-29 - Grade4 Component Causal Triage Script

Added:

```text
scripts/analysis_tools/grade4_component_causal_triage.py
```

Purpose:

```text
Small post-analyzer/post-run readout for the causal x_order_orth vs x_content
question. It is not a new experiment and not a second full analyzer. It reads
existing Grade4 component-causal CSVs and produces a compact verdict, paired
order-vs-content table, alpha-scaling summary, rank summary, and transparent
criteria JSON.
```
...
```powershell
python scripts/analysis_tools/grade4_component_causal_triage.py `
  --input "<PATH_TO_NEW_CAUSAL_RUN_ZIP_OR_DIR_OR_ANALYZER_OUTPUT>" `
  --output-dir "research_synthesis/gemma3_grade4_sae_academic_readout/causal_xorder_triage" `
  --force-extract
```
...
```text
grade4_causal_triage_report.md
grade4_causal_triage_verdict.json
grade4_causal_triage_criteria.json
grade4_causal_triage_axis_summary.csv
grade4_causal_triage_order_vs_content.csv
grade4_causal_triage_alpha_summary.csv
grade4_causal_triage_rank_summary.csv
```

Boundary:

```text
The script has predeclared evaluation criteria, not a predeclared outcome. It
can return strong, partial, weak/inconclusive, content-stronger, or missing-data
verdicts. It does not rewrite metrics or generate evidence not present in the
input artifacts.
```

## 2026-05-29 - Gemma3 12B IT Causal x_order Run Result
...
```text
The new Gemma3-12B-IT Grade 4 component-causal run confirms that x_order_orth is
causally active: plus/minus interventions along this axis modulate the
generation trajectory with a positive mean gap and high positive-gap rate.

However, the run does not support the stronger claim that x_order_orth is the
dominant causal component. In the current raw-alpha setup, x_content has a
larger mean causal gap and beats x_order_orth in most paired cells.

The correct claim upgrade is limited: descriptive separability -> causal
involvement, not causal dominance.
```

Main confound:

```text
The current intervention adds alpha times raw component vectors:
tensor + alpha * vec

At equal alpha, x_content receives a larger effective perturbation because its
band norm is larger:
...
```text
Run norm-controlled component-causal intervention before behavioral steering.
Normalize x_order_orth and x_content by intervention-band norm, then repeat the
same plus/minus alpha sweep. Only after equal-energy comparison should we decide
whether to move into behavioral steering.
```
