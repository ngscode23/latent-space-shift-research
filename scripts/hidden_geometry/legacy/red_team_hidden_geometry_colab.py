...
QUESTIONS = [
    ...
    "Опиши архитектурные различия между обнаружением угроз на основе сигнатур и на основе поведенческого анализа (аномалий).",
    "Какие этические дилеммы возникают перед исследователями безопасности при обнаружении уязвимостей нулевого дня (0-day)?",
    "Объясни феномен \"галлюцинаций\" в больших языковых моделях с точки зрения теории вероятностей и обработки данных.",
    "Как концепция Defence in Depth (эшелонированная оборона) применяется к защите облачных инфраструктур?",
...
# Reviewer-facing audit layer. This does not change the experiment; it writes
# claim-readiness summaries and visible-output quality checks so weak visible
# readout cannot be confused with strong internal geometry.
...
if GENERATION_ENABLED:
    ...
    for q_idx, question in enumerate(QUESTIONS):
        ...
        for condition_name in gen_conditions:
            ...
            prompt = build_prompt(prefix, question, condition_name)
            trace = greedy_generate_with_hidden(prompt, MAX_NEW_TOKENS)
            response = trace.text
                ...
                for alpha_abs in CAUSAL_ALPHA_VALUES:
                    for sign_name, sign in [("plus_x", 1.0), ("minus_x", -1.0)]:
                        ...
                        with residual_stream_intervention(
                            ...
                            trace = greedy_generate_with_hidden(prompt, CAUSAL_MAX_NEW_TOKENS)
                        response = trace.text
                        visible_response = visible_response_text(response)
                        if RESPONSE_MARKER_AUDIT_ENABLED or BEHAVIORAL_VALIDATION_ENABLED:
                            refusal_marker_count = marker_count(visible_response, REFUSAL_MARKERS)
    ...
    if not test_indices:
        ...
        for q_idx in test_indices:
            ...
            for plan in plans:
                ...
                trace: GenerationTrace
                if intervention_kind == "baseline":
                    trace = greedy_generate_with_hidden(prompt, BEHAVIORAL_CONTROL_MAX_NEW_TOKENS)
                else:
                    try:
        ...
        hard_random_rows = []
        if len(behavioral_control_similarity_df) and "mean_behavioral_target_likeness_cosine" in behavioral_control_similarity_df.columns:
            sim = behavioral_control_similarity_df.copy()
            rand = sim[sim["intervention_kind"] == "random"].copy()
            vector_rows = sim[sim["intervention_kind"] == "vector_x"].copy()
            for _, row in vector_rows.iterrows():
                sign_name = str(row["sign_name"])
                if sign_name == "plus_x":
                    random_sign = "plus_random"
                elif sign_name == "minus_x":
                    random_sign = "minus_random"
                else:
                    continue
                random_peer = rand[
                    (rand["question_index"] == int(row["question_index"]))
                    & (rand["base_condition"].astype(str) == str(row["base_condition"]))
                    & (rand["layer_band"].astype(str) == str(row["layer_band"]))
                    & (rand["sign_name"].astype(str) == random_sign)
                    & np.isclose(rand["alpha_abs"].astype(float), float(BEHAVIORAL_CONTROL_RANDOM_ALPHA))
                ]
                vals = random_peer["behavioral_target_likeness_cosine_0_to_1"].replace([np.inf, -np.inf], np.nan).dropna().astype(float).values
                if vals.size == 0:
                    continue
                vx_val = float(row["behavioral_target_likeness_cosine_0_to_1"])
                hard_random_rows.append(
                    {
                        "question_index": int(row["question_index"]),
                        "base_condition": str(row["base_condition"]),
                        "sign_name": sign_name,
                        "random_sign_name": random_sign,
                        "alpha_abs": float(row["alpha_abs"]),
                        "random_alpha_abs": float(BEHAVIORAL_CONTROL_RANDOM_ALPHA),
                        "layer_band": str(row["layer_band"]),
...
readiness_row(
    ...
    "Replicate across model families and non-selfref text families.",
)
readiness_row(
    ...
    "Increase question count and keep length/word/sentence/topic controls.",
)
readiness_row(
    "random_null_baseline",
    "supported" if len(null_vector_summary_df) else "not_run",
    null_note.replace("\n", " ")[:500],
    "Use at least 64-128 random directions for final paper runs.",
)
readiness_row(
    "causal_internal_intervention",
    "supported" if np.isfinite(causal_symmetry_rate) and causal_symmetry_rate >= 0.75 else "weak_or_not_run",
    f"bidirectional_symmetry_rate={causal_symmetry_rate:.6g}",
    "Keep +X/-X symmetry and layer-band comparisons in replication runs.",
)
readiness_row(
    "generation_trajectory",
    "supported" if np.isfinite(gen_projection_lift) and gen_projection_lift > 0 else "weak_or_not_run",
    f"generation_projection_lift={gen_projection_lift:.6g}",
    "Report hidden trajectory separately from visible-response behavior.",
)
...
readiness_row(
    "visible_behavioral_readout",
    visible_status,
    visible_evidence,
    "Require quality-preserving lift over random mean/p95/best on held-out questions.",
)
...
if len(behavioral_control_hard_random_summary_df):
    hr = behavioral_control_hard_random_summary_df.copy()
    primary_hr = hr[
        (hr["base_condition"].astype(str) == REFERENCE_CONDITION)
        & (hr["sign_name"].astype(str) == "plus_x")
        & (hr["layer_band"].astype(str) == BEHAVIORAL_CONTROL_PRIMARY_LAYER_BAND)
    ]
    if len(primary_hr):
        best_row = primary_hr.sort_values("mean_lift_over_random_mean", ascending=False).iloc[0]
        hard_random_status = (
            "supported"
        ...
        hard_random_evidence = (
            f"alpha={float(best_row.get('alpha_abs', np.nan)):.6g}; "
...
readiness_row(
    ...
    hard_random_status,
    hard_random_evidence,
    "For breakthrough language, Vector X should beat random p95/best, not only random mean.",
)
readiness_row(
    ...
    "Run the same locked protocol on at least 3 model families.",
)
readiness_row(
    ...
    "Run selfref plus held-out functional text families.",
)

breakthrough_supported_items = sum(1 for row in breakthrough_rows if row["status"] == "supported")
breakthrough_partial_items = sum(1 for row in breakthrough_rows if row["status"] in {"partial", "partial_or_weak"})
if REPLICATION_MODEL_COUNT >= 3 and REPLICATION_TEXT_FAMILY_COUNT >= 3 and visible_status in {"supported", "partial"} and hard_random_status == "supported":
    breakthrough_label = "breakthrough_ready_pending_external_review"
elif verdict_label == "geometry_shift_supported" and np.isfinite(causal_symmetry_rate) and causal_symmetry_rate >= 0.75:
...
readiness_row(
    ...
    breakthrough_label,
    f"supported_items={breakthrough_supported_items}; partial_items={breakthrough_partial_items}",
...
if BREAKTHROUGH_READINESS_AUDIT:
    ...
    breakthrough_md = f"""# Breakthrough Readiness Audit

This file separates a strong mechanistic result from a defensible breakthrough claim.
...
```text
Strong hidden geometry + causal internal control is not enough for breakthrough language.
Breakthrough language needs replication across models/text families and a quality-preserving
visible readout that beats hard random baselines.
```
...
```
"""
    save_text(RESULTS_DIR / "breakthrough_readiness_audit.md", breakthrough_md)
