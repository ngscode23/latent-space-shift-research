...
EMIT_NARRATIVE = env_bool("EMIT_NARRATIVE", False)
...
# Backward-compatible env name, but generated CSVs are now dry evidence tables
# and markdown/text narrative is suppressed unless EMIT_NARRATIVE=True.
EVIDENCE_PACKAGE_AUDIT = env_bool(
    "EVIDENCE_PACKAGE_AUDIT",
    True,
)
...
FORBIDDEN_NARRATIVE_COLUMNS = {
    ...
    "claim_if_supported",
    "mechanistic_meaning",
    "next_experiment_or_requirement",
    "overall_claim_boundary",
    "why_it_matters",
}
FORBIDDEN_RESULT_LABEL_VALUES = {
    "breakthrough",
    "breakthrough_ready_pending_external_review",
    "strong_mechanistic_case_not_breakthrough_yet",
    "promising_latent_shift_case_needs_causal_and_control_hardening",
    "preliminary_or_incomplete",
...
def is_narrative_artifact_path(path: Path) -> bool:
    ...
    if path.suffix.lower() in {".md", ".txt"} and name not in {"metric_math_reference.md"}:
        return True
    return "verdict" in name or "readiness_report" in name


def evidence_output_path(path: Path) -> Optional[Path]:
    path = Path(path)
    mapped_name = OUTPUT_FILENAME_MAP.get(path.name, path.name)
    path = path.with_name(mapped_name)
    if is_under_path(path, RESULTS_DIR) and is_narrative_artifact_path(path):
...
def infer_artifact_type_from_name(filename: str) -> str:
    name = filename.lower()
    if "manifest" in name or "metadata" in name or "metric_notes" in name:
        return "protocol_reference"
    if any(key in name for key in ["raw", "turns", "audit", "outputs", "inputs", "tasks", "tokens", "layers_tested", "vectors"]):
        return "raw_measurement"
    if any(key in name for key in ["threshold", "criteria", "scorecard", "validity", "gate", "condition_summary"]):
        return "threshold_eval"
    if any(key in name for key in ["not_available", "missing", "negative_results"]):
        return "coverage_or_missingness"
    if any(key in name for key in ["proxy", "behavior", "marker"]):
        return "proxy_metric"
    return "derived_metric"


def failure_code_from_status_or_pass(status, passed=None) -> str:
    status_s = "" if status is None else str(status)
    if status_s in {"not_tested", "not_available"} or status_s.startswith("not_available"):
        return "missing_required_metric"
    if status_s.startswith("missing"):
        return status_s
    if status_s in {"not_supported_or_mixed", "not_supported_or_insufficient_n", "missing_external_replication"}:
...
def prepare_evidence_dataframe_for_write(df: pd.DataFrame, filename: str) -> pd.DataFrame:
    out = df.copy()
    artifact_type = infer_artifact_type_from_name(filename)

    drop_cols = [
        col for col in out.columns
        if str(col).lower() in FORBIDDEN_NARRATIVE_COLUMNS
    ]
    if drop_cols:
        out = out.drop(columns=drop_cols)

    if "artifact_type" not in out.columns:
        out.insert(0, "artifact_type", artifact_type)
    else:
        out["artifact_type"] = out["artifact_type"].fillna(artifact_type)
        out.loc[~out["artifact_type"].astype(str).isin(ALLOWED_ARTIFACT_TYPES), "artifact_type"] = artifact_type

    if filename in {"claim_threshold_eval.csv", "evidence_threshold_scorecard.csv"}:
        out["artifact_type"] = "threshold_eval"
        if "criterion" in out.columns:
            out = out[out["criterion"].astype(str) != "overall_interpretation"].copy()
        if "pass" not in out.columns and "status" in out.columns:
            out["pass"] = out["status"].astype(str).eq("supported").astype(int)
        if "status" in out.columns:
            out["status"] = out["status"].astype(str).replace({
                "supported": "computed",
                "partial": "computed",
                "summary": "computed",
                "not_supported_or_mixed": "computed",
                "not_supported_or_insufficient_n": "computed",
                "not_tested": "not_available",
                "missing_external_replication": "not_available",
            })
    if "failure_code" not in out.columns and ("status" in out.columns or "pass" in out.columns):
        out["failure_code"] = [
            failure_code_from_status_or_pass(row.get("status", ""), row.get("pass", None))
            for _, row in out.iterrows()
        ]

    for col in list(out.columns):
        col_l = str(col).lower()
        if col_l in TEXT_COLUMNS_ALLOWED_TO_KEEP_RAW:
            continue
        if out[col].dtype == object:
            out[col] = out[col].map(
                lambda value: ""
                if str(value).strip() in FORBIDDEN_RESULT_LABEL_VALUES
                else value
            )
    return out
...
def save_json(path: Path, data):
    out_path = evidence_output_path(path)
    if out_path is None:
        return
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if isinstance(data, dict) and is_under_path(out_path, RESULTS_DIR) and "artifact_type" not in data:
        data = dict(data)
        data["artifact_type"] = infer_artifact_type_from_name(out_path.name)
    out_path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    remember_artifact(out_path)
...
def save_text(path: Path, text: str):
    ...
    if out_path is None:
        return
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if is_narrative_artifact_path(out_path):
        text = AUTOGENERATED_INTERPRETATION_HEADER + text
    out_path.write_text(text, encoding="utf-8")
    remember_artifact(out_path)


def save_df(df: pd.DataFrame, filename: str):
    mapped_name = OUTPUT_FILENAME_MAP.get(filename, filename)
    path = RESULTS_DIR / mapped_name
    out_path = evidence_output_path(path)
    if out_path is None:
        return
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df = prepare_evidence_dataframe_for_write(df, out_path.name)
...
run_metadata = {
    ...
    "emit_narrative": EMIT_NARRATIVE,
    "analysis_notes_dir": str(NARRATIVE_DIR),
    "evidence_package_audit": EVIDENCE_PACKAGE_AUDIT,
...
df_claim_threshold_eval = pd.DataFrame(checklist_rows)
save_df(df_claim_threshold_eval, "claim_threshold_eval.csv")
claim_threshold_eval_text = df_claim_threshold_eval.to_string(index=False)
...
def build_vector_x_rlhf_proxy_notes() -> str:
    # Optional human-readable notes. CSV threshold files remain the source of truth.
    ...
    if df_vector_x_rlhf_proxy_transfer_summary.empty:
        ...
        status_code = "computed_all_proxy_thresholds_pass"
        threshold_note = (
            "All proxy threshold checks passed for this run."
        )
    elif (
        np.isfinite(natural_gap)
        and natural_gap > 0
        ...
        status_code = "computed_partial_proxy_thresholds_pass"
        threshold_note = (
            "Some proxy threshold checks passed; inspect threshold CSV rows."
        )
    elif np.isfinite(natural_gap) and natural_gap > 0:
        status_code = "computed_natural_gap_only"
        threshold_note = (
            "Natural proxy gap was positive; transfer thresholds did not all pass."
        )
    else:
        status_code = "computed_thresholds_not_passed"
        threshold_note = (
            "Proxy threshold checks did not pass in this run."
        )
    ...
    lines = [
        "# Vector X RLHF/Safety Proxy Notes",
        "",
        "## Threshold Note",
        "",
        threshold_note,
...
vector_x_rlhf_proxy_notes_text = build_vector_x_rlhf_proxy_notes()
save_text(RESULTS_DIR / "vector_x_rlhf_proxy_notes.md", vector_x_rlhf_proxy_notes_text)


def build_vector_x_rlhf_threshold_eval() -> pd.DataFrame:
    natural_gap = _first_summary_float(
        df_vector_x_rlhf_proxy_transfer_summary,
        "mean_abs_natural_direct_gap",
    )
    ...
    def add_row(criterion: str, metric_name: str, metric_value: float, threshold_value: float = 0.0) -> None:
        is_available = np.isfinite(metric_value)
        passed = bool(is_available and metric_value > threshold_value)
        rows.append({
            "criterion": criterion,
            "metric_name": metric_name,
            "metric_value": metric_value if is_available else float("nan"),
            "threshold_operator": ">",
            "threshold_value": threshold_value,
            "pass": int(passed) if is_available else 0,
            "status": "computed" if is_available else "not_available",
            "autoscored": True,
            "failure_code": "" if passed else ("below_threshold" if is_available else "missing_required_metric"),
        })

    add_row("natural_proxy_gap_positive", "mean_abs_natural_direct_gap", natural_gap)
    add_row("candidate_beats_control_slope", "candidate_minus_control_dose_slope", slope_delta)
    add_row("candidate_positive_fraction_positive", "candidate_minus_control_positive_fraction", positive_delta)
    add_row("candidate_slope_positive", "best_candidate_dose_slope", candidate_slope)
    return pd.DataFrame(rows)


save_df(build_vector_x_rlhf_threshold_eval(), "vector_x_rlhf_proxy_threshold_eval.csv")
...
def _audit_checklist_status(criterion: str) -> str:
    if not _audit_df_available(df_claim_threshold_eval):
        return "not_tested"
    rows = df_claim_threshold_eval[
        df_claim_threshold_eval["criterion"].astype(str) == str(criterion)
    ]
    if rows.empty:
        return "not_tested"
...
def _audit_partial(status: str) -> bool:
    return str(status) in {"supported", "partial"}
...
def build_evidence_package_audit() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict[str, object], str]:
    """Machine-readable scorecard for run-level evidence coverage."""
    ...
    if _audit_df_available(df_claim_threshold_eval):
        for _, row in df_claim_threshold_eval.iterrows():
            status = str(row.get("status", ""))
            if status not in {"supported", "summary"}:
                negative_rows.append({
                "source": "claim_threshold_eval",
    ...
    for _, row in scorecard_df.iterrows():
        ...
        if status not in {"supported", "partial"}:
            negative_rows.append({
                "source": "evidence_threshold_scorecard",
    ...
    if geometry_ok and semantic_ok and causal_ok and controls_ok and visible_ok and replication_ok and strict_ok:
        overall_status = "computed_all_thresholds_pass"
        overall_failure_code = ""
    elif geometry_ok and semantic_ok and causal_ok and controls_ok:
        overall_status = "computed_core_thresholds_pass"
        overall_failure_code = "missing_external_replication_or_strict_gate"
    elif geometry_ok and semantic_ok:
        overall_status = "computed_partial_thresholds_pass"
        overall_failure_code = "missing_causal_or_control_threshold"
    else:
        overall_status = "computed_incomplete"
        overall_failure_code = "missing_primary_threshold"
    ...
    report = f"""# Evidence Package Audit
...
controls, and predictive validity all point in the same direction. Article-level
claims require independent replication across models and stimulus families.
...
evidence_scorecard_df = pd.DataFrame()
evidence_predictive_df = pd.DataFrame()
negative_results_df = pd.DataFrame()
evidence_audit_notes_text = "Evidence package audit disabled."
if EVIDENCE_PACKAGE_AUDIT:
    (
        evidence_scorecard_df,
        evidence_predictive_df,
        negative_results_df,
        replication_manifest,
        evidence_audit_notes_text,
    ) = build_evidence_package_audit()
    save_df(evidence_scorecard_df, "evidence_threshold_scorecard.csv")
    save_df(evidence_predictive_df, "evidence_predictive_validity.csv")
    ...
    save_text(RESULTS_DIR / "evidence_package_audit_notes.md", evidence_audit_notes_text)
...
actual_artifact_names = sorted(set(SAVED_ARTIFACTS))
...
Vector X RLHF/safety proxy notes:
{vector_x_rlhf_proxy_notes_text}
...
if EMIT_NARRATIVE:
    print("saved:", NARRATIVE_DIR / "summary_report.txt")
...
if COPY_AB_SEMANTIC_KEY_FILES and AB_SEMANTIC_STEERING_ANALYSIS:
    ...
    save_json(key_dir / "manifest.json", key_manifest)
...
def validate_evidence_package_schema(results_dir: Path) -> None:
    errors = []
    run_name = str(results_dir)
    if any(fragment in run_name.lower() for fragment in FORBIDDEN_RESULT_NAME_FRAGMENTS):
        errors.append(f"RESULTS_DIR contains result-branding fragment: {results_dir}")

    root_files = [path for path in results_dir.glob("*") if path.is_file()]
    if not EMIT_NARRATIVE:
        narrative_files = [
            path for path in root_files
            if "verdict" in path.name.lower()
            or "readiness_report" in path.name.lower()
            or path.name.lower() == "summary_report.txt"
        ]
        if narrative_files:
            errors.extend(f"Narrative artifact exists in root while EMIT_NARRATIVE=False: {path}" for path in narrative_files)

    if EMIT_NARRATIVE and NARRATIVE_DIR.exists():
        for path in NARRATIVE_DIR.glob("*"):
            if path.is_file() and path.suffix.lower() in {".md", ".txt"}:
                try:
                    first_line = path.read_text(encoding="utf-8").splitlines()[0]
                except Exception:
                    first_line = ""
                if first_line != AUTOGENERATED_INTERPRETATION_HEADER.strip():
                    errors.append(f"Narrative file missing autogenerated header: {path}")
    ...
    for csv_path in results_dir.rglob("*.csv"):
        try:
            ...
            errors.append(f"Could not read CSV for schema validation: {csv_path}: {exc!r}")
            continue
        if "artifact_type" not in df.columns:
            errors.append(f"CSV missing artifact_type column: {csv_path}")
        bad_cols = [
            col for col in df.columns
            if str(col).lower() in FORBIDDEN_NARRATIVE_COLUMNS
        ]
        if bad_cols:
            errors.append(f"CSV contains narrative columns {bad_cols}: {csv_path}")
        for col in df.columns:
            if str(col).lower() in TEXT_COLUMNS_ALLOWED_TO_KEEP_RAW:
                continue
            values = set(str(v).strip() for v in df[col].dropna().astype(str).head(500))
            bad_values = sorted(values & FORBIDDEN_RESULT_LABEL_VALUES)
            if bad_values:
                errors.append(f"CSV contains forbidden label values {bad_values} in column {col}: {csv_path}")
        if "status" in df.columns and "failure_code" in df.columns:
            mask = df["status"].astype(str).str.startswith("not_available")
            if bool(mask.any()):
                missing_failure = df.loc[mask, "failure_code"].fillna("").astype(str).eq("")
                if bool(missing_failure.any()):
                    errors.append(f"not_available rows without failure_code: {csv_path}")
    ...
    for json_path in results_dir.rglob("*.json"):
        try:
            ...
            continue
        if isinstance(payload, dict) and "artifact_type" not in payload:
            errors.append(f"JSON missing artifact_type: {json_path}")
        if isinstance(payload, dict):
            for key in ["overall_claim_boundary", "claim_boundary", "note"]:
                if key in payload:
                    errors.append(f"JSON contains narrative key {key}: {json_path}")
...
if EMIT_NARRATIVE:
    print("- Narrative notes were written under analysis_notes/. Treat them as interpretation, not raw evidence.")
else:
