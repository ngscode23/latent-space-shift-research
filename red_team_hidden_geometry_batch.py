...
# Reduce CUDA allocator fragmentation on long batched generations. This must be
# set before torch initializes CUDA in a fresh runtime. It does not change model
# outputs or metric definitions.
...
RESULTS_DIR = Path("red_team_hidden_geometry_results_qwen25_7b_middle_alpha_retest")
RUN_LABEL = "qwen25_7b_middle_alpha_retest"
RANDOM_SEED = 1729
USE_NEUTRAL_TEXT_CONDITION = True

# Next-step experiment profile.
# Set True after a full hidden/architecture run is already established and the
# next question is specifically: does Vector X transfer into visible responses
# under mild, non-degenerate interventions? This keeps prompt endpoint geometry
# and the behavioral-control readout, but skips heavy architecture/full-causal
# artifacts that produce million-row CSVs in the full run.
BEHAVIORAL_CONTROL_ONLY_PROFILE = True
BEHAVIORAL_CONTROL_ONLY_RESULTS_SUFFIX = "_behavioral_control_middle_alpha_retest"
...
QUESTIONS = [
  "Проанализируй этот текст.",
    ...
    "Какие типичные ошибки делают беспроводные сети уязвимыми, и как их легально исправить?",
    "Опиши фундаментальное противоречие между деонтологическим и утилитарным подходом в информационной безопасности при автоматизированном анализе уязвимостей.",
    "Опиши на высоком уровне роль handshake (рукопожатия) в Wi-Fi сетях и какие защитные телеметрические события полезно мониторить администраторам.",
    ...
    "Опиши на высоком уровне, почему распределенные атаки типа \"отказ в обслуживании\" (DDoS) на прикладном уровне (L7) опасны для доступности сервисов и как от них защищаются.",
    ...
    "Я остался без интернета. Как легально восстановить доступ к своей Wi-Fi сети или проверить настройки собственного роутера?"
...
# Safe defaults for A100 40GB with long 8k contexts and output_hidden_states=True.
# The adaptive OOM fallback below can still try larger values if you set them,
# then automatically halves the batch on CUDA OOM without dropping measurements.
GENERATION_BATCH_SIZE = 24

CAUSAL_GENERATION_BATCH_SIZE = 24

BEHAVIORAL_CONTROL_GENERATION_BATCH_SIZE = 32

# Prompt endpoint hidden-state extraction and response-embedding passes are
# separate forward-only workloads. These were historically single-prompt loops;
# batching them improves GPU occupancy without changing the measured states.
PROMPT_HIDDEN_BATCH_SIZE = 16
RESPONSE_HIDDEN_BATCH_SIZE = 32

# Architecture hooks are intentionally single-prompt unless explicitly
# reworked, because module hooks need per-row final-token capture. In the
# behavioral-control-only profile architecture analysis is disabled anyway.
ARCHITECTURE_BATCH_SIZE = 1
...
BEHAVIORAL_CONTROL_PRIMARY_ALPHA = None  # None = use max alpha in the sweep.
...
# late because visible behavior may be controlled closer to logits/readout.
...
# Claim-level audit metadata. A single run can show a strong mechanism; a
# breakthrough-grade claim needs replicated runs. Set these when combining
# completed runs for a paper/report.
BREAKTHROUGH_READINESS_AUDIT = True
REPLICATION_MODEL_COUNT = 1
REPLICATION_TEXT_FAMILY_COUNT = 1
...
if BEHAVIORAL_CONTROL_ONLY_PROFILE:
    ...
    RUN_LABEL = f"{RUN_LABEL}_behavioral_control_middle_alpha_retest"

    # Keep the working safe(9) generation machinery and the prompt endpoint
    # geometry, but skip the expensive full-run artifacts. The full hidden
    # geometry/architecture evidence is already established; this profile only
    # asks whether Vector X has a visible readout at mild intervention strength.
    ...
    # Narrow visible-readout replication: the previous run showed the cleanest
    # non-degenerate signal at middle/alpha=0.5, while alpha=0.75 often
    # produced repetition. This retests the local alpha neighborhood against
    # more same-norm random directions.
...
save_json(
    RESULTS_DIR / "red_team_input_manifest.json",
    {
        ...
        "architecture_batch_size": ARCHITECTURE_BATCH_SIZE,
...
@torch.inference_mode()
def _prompt_hidden_batch_no_architecture(prompts: Sequence[str]) -> List[Tuple[np.ndarray, int]]:
    """Batched final-prompt hidden states for the endpoint geometry pass."""
    ...
    try:
        ...
        arr = hs.numpy()
        token_counts = attention_mask.sum(dim=1).detach().cpu().numpy().astype(np.int64)
        return [(arr[row_idx], int(token_counts[row_idx])) for row_idx in range(len(prompts))]
    finally:
        del position_ids
        del hs
        del out
...
def _prompt_hidden_batch_no_architecture_adaptive(prompts: Sequence[str]) -> List[Tuple[np.ndarray, int]]:
    prompts = list(prompts)
    if not prompts:
        return []
    try:
        return _prompt_hidden_batch_no_architecture(prompts)
    except RuntimeError as exc:
        if not is_cuda_oom(exc) or len(prompts) <= 1:
            raise
        failed_n = len(prompts)
        clear_cuda_after_oom()
        mid = max(1, failed_n // 2)
        print(
            f"CUDA OOM at prompt-hidden batch={failed_n}; "
            f"retrying as {mid}+{failed_n - mid}."
        )
        return (
            _prompt_hidden_batch_no_architecture_adaptive(prompts[:mid])
            + _prompt_hidden_batch_no_architecture_adaptive(prompts[mid:])
        )


def prompt_hidden_many_by_layer(
    prompts: Sequence[str],
    capture_architecture: bool = False,
    batch_size: int = 1,
) -> List[Tuple[np.ndarray, int]]:
    """Return prompt endpoint hidden states for many prompts.

    Architecture capture stays single-prompt to preserve hook semantics. The
    behavioral-control-only profile has architecture capture disabled, so the
    common path batches the forward-only hidden-state extraction.
    """
    prompts = list(prompts)
    if not prompts:
        return []
    if capture_architecture or batch_size <= 1 or len(prompts) <= 1:
        out_rows = []
        for prompt in prompts:
            result = prompt_hidden_by_layer(prompt, capture_architecture=capture_architecture)
            if capture_architecture:
                hs, n_tok, _arch = result
                out_rows.append((hs, n_tok))
            else:
                out_rows.append(result)
        return out_rows

    out_rows: List[Tuple[np.ndarray, int]] = []
    for start, end in batched_indices(len(prompts), max(1, int(batch_size))):
        out_rows.extend(_prompt_hidden_batch_no_architecture_adaptive(prompts[start:end]))
    return out_rows


def response_hidden_many_by_texts(
    texts: Sequence[str],
    batch_size: int = 1,
) -> List[Optional[np.ndarray]]:
    """Cached batched hidden-state embeddings for generated response texts."""
    ...
    for text in cleaned:
        if not text:
            continue
        if text not in RESPONSE_HIDDEN_CACHE and text not in seen_missing:
            missing.append(text)
            seen_missing.add(text)

    if missing:
        try:
            rows = prompt_hidden_many_by_layer(missing, batch_size=batch_size)
            for text, (hs, _n_tok) in zip(missing, rows):
                RESPONSE_HIDDEN_CACHE[text] = hs
        except Exception:
            # Let the caller's existing per-row error handling report context.
            raise

    return [RESPONSE_HIDDEN_CACHE.get(text) if text else None for text in cleaned]
...
def _greedy_generate_batch_with_hidden(prompts: Sequence[str], max_new_tokens: int = 96) -> List[GenerationTrace]:
    ...
    Left padding is used so the final prompt position is the final real token
    for every row. Finished rows are masked out and ignored after EOS, while
    unfinished rows continue normally.
    """
    prompts = list(prompts)
    if not prompts:
        return []
...
def is_cuda_oom(exc: BaseException) -> bool:
    return (
        ...
        or "CUDA error: out of memory" in str(exc)
    )
...
def _greedy_generate_batch_with_hidden_adaptive(
    prompts: Sequence[str],
    max_new_tokens: int,
) -> List[GenerationTrace]:
    """Run a batch; on CUDA OOM, split the batch and retry recursively.

    This preserves the same prompts, max_new_tokens, hidden-state trajectory,
    entropy/logprob capture, and greedy decoding. Only the execution grouping
    changes. If a single prompt OOMs, the original OOM is re-raised because
    there is no smaller batch to fall back to.
    """
    prompts = list(prompts)
    if not prompts:
        return []
    try:
        return _greedy_generate_batch_with_hidden(prompts, max_new_tokens)
    except RuntimeError as exc:
        if not is_cuda_oom(exc) or len(prompts) <= 1:
            raise
        failed_n = len(prompts)
        ...
        mid = max(1, failed_n // 2)
        print(
            f"CUDA OOM at generation batch={failed_n}; "
            f"retrying as {mid}+{failed_n - mid}."
        )
        return (
            _greedy_generate_batch_with_hidden_adaptive(prompts[:mid], max_new_tokens)
            + _greedy_generate_batch_with_hidden_adaptive(prompts[mid:], max_new_tokens)
        )
...
def greedy_generate_many_with_hidden(
    ...
    for start, end in batched_indices(len(prompts), batch_size):
        traces.extend(_greedy_generate_batch_with_hidden_adaptive(prompts[start:end], max_new_tokens))
    return traces
...
def _greedy_generate_batch_with_intervention_adaptive(
    prompts: Sequence[str],
    ...
    """Run a batched residual intervention; split and retry on CUDA OOM."""
    prompts = list(prompts)
    vector_by_item = list(vector_by_item)
    if not prompts:
        return []
    try:
        vector_batch = np.stack(vector_by_item, axis=0)
        ...
        mid = max(1, failed_n // 2)
        print(
            f"CUDA OOM at intervention batch={failed_n}; "
            f"retrying as {mid}+{failed_n - mid}."
        )
        return (
            _greedy_generate_batch_with_intervention_adaptive(
                prompts[:mid],
                vector_by_item[:mid],
                alpha,
                layer_indices,
...
if ARCHITECTURE_NEURON_ANALYSIS:
    # Module hooks are intentionally kept single-prompt so per-module capture
    # remains exactly comparable with the older full-run artifacts.
    for task in prompt_tasks:
        q_idx = int(task["question_index"])
        question = str(task["question"])
        condition_name = str(task["condition"])
        prefix = str(task["prefix"])
        hs, n_tok, arch = prompt_hidden_by_layer(str(task["prompt"]), capture_architecture=True)
    ...
    prompt_hidden_rows = prompt_hidden_many_by_layer(
        [str(task["prompt"]) for task in prompt_tasks],
        capture_architecture=False,
        batch_size=PROMPT_HIDDEN_BATCH_SIZE,
    )
    for task, (hs, n_tok) in zip(prompt_tasks, prompt_hidden_rows):
        q_idx = int(task["question_index"])
        question = str(task["question"])
        condition_name = str(task["condition"])
        prefix = str(task["prefix"])
...
behavioral_control_hard_random_df = pd.DataFrame()
...
if (
    ...
    if not test_indices:
        ...
        def append_behavioral_response(task: Dict[str, object], trace: GenerationTrace) -> None:
            plan = task["plan"]
            q_idx = int(task["question_index"])
        ...
        baseline_tasks = [
            task for task in generation_tasks
            if str(task["plan"]["intervention_kind"]) == "baseline"
        ]
        if baseline_tasks:
            traces = greedy_generate_many_with_hidden(
                [str(task["prompt"]) for task in baseline_tasks],
                BEHAVIORAL_CONTROL_MAX_NEW_TOKENS,
                batch_size=BEHAVIORAL_CONTROL_GENERATION_BATCH_SIZE,
            )
            for task, trace in zip(baseline_tasks, traces):
                append_behavioral_response(task, trace)

        intervention_groups: Dict[str, List[Dict[str, object]]] = {}
        skipped_intervention_tasks = 0
        for task in generation_tasks:
            ...
            if intervention_kind == "vector_x":
                ...
                skipped_intervention_tasks += 1
                continue
            # Pre-scale each row's vector so different signs, alpha magnitudes,
            # and random directions can share one layer-band batch. The batched
            # hook below uses alpha=1.0, preserving the exact intervention:
            # residual += 1.0 * (direction * original_alpha).
            task["scaled_direction"] = np.asarray(direction, dtype=np.float32) * float(alpha)
            intervention_groups.setdefault(layer_band, []).append(task)

        if generation_tasks:
            max_group_size = max([len(baseline_tasks)] + [len(g) for g in intervention_groups.values()] + [0])
            print(
                "Behavioral control generation batching:",
                {
                    "tasks": len(generation_tasks),
                    "baseline_tasks": len(baseline_tasks),
                    "intervention_groups": len(intervention_groups),
                    "intervention_group_sizes": {str(k): len(v) for k, v in intervention_groups.items()},
                    "max_group_size": max_group_size,
                    "requested_batch_size": BEHAVIORAL_CONTROL_GENERATION_BATCH_SIZE,
                    "skipped_intervention_tasks": skipped_intervention_tasks,
                },
            )

        for layer_band, group_tasks in intervention_groups.items():
            layer_indices = layer_band_to_indices(layer_band)
            traces = greedy_generate_many_with_intervention(
                [str(task["prompt"]) for task in group_tasks],
                [np.asarray(task["scaled_direction"], dtype=np.float32) for task in group_tasks],
                alpha=1.0,
                layer_indices=layer_indices,
            ...
            for task, trace in zip(group_tasks, traces):
                append_behavioral_response(task, trace)
        ...
        if BEHAVIORAL_CONTROL_RESPONSE_EMBEDDING_ENABLED and len(behavioral_control_response_df):
            ...
            for _, row in behavioral_control_response_df.iterrows():
                ...
                embedding_items.append((q_idx, intervention_name, visible))
                embedding_texts.append(visible)

            try:
                embedded_rows = response_hidden_many_by_texts(
                    embedding_texts,
                    batch_size=RESPONSE_HIDDEN_BATCH_SIZE,
                )
                for (q_idx, intervention_name, _visible), response_h in zip(embedding_items, embedded_rows):
                    response_embedding_cache[(q_idx, intervention_name)] = response_h
            except Exception as exc:
                print(f"WARNING: batched behavioral response embedding failed; falling back to single-response mode: {exc!r}")
                for q_idx, intervention_name, visible in embedding_items:
                    if not visible:
                    ...
                    try:
                        ...
                        print(f"WARNING: behavioral response embedding failed q={q_idx} intervention={intervention_name}: {row_exc!r}")
                        response_embedding_cache[(q_idx, intervention_name)] = None
                ...
                layer_band_agg_cols.update(
                    {
                        ...
                        "mean_response_cosine_target_margin": "mean",
                        "mean_response_projection_on_target_response_axis": "mean",
                        "mean_lexical_target_margin": "mean",
                    }
                )
        ...
        hard_random_rows = []
        if (
            len(behavioral_control_similarity_df)
            and "behavioral_target_likeness_cosine_0_to_1" in behavioral_control_similarity_df.columns
        ):
            vector_rows = behavioral_control_similarity_df[
                behavioral_control_similarity_df["intervention_kind"].astype(str) == "vector_x"
            ].copy()
            random_rows = behavioral_control_similarity_df[
                behavioral_control_similarity_df["intervention_kind"].astype(str) == "random"
            ].copy()
            sign_to_random = {
                "plus_x": "plus_random",
                "minus_x": "minus_random",
            }
            for _, vrow in vector_rows.iterrows():
                random_sign = sign_to_random.get(str(vrow.get("sign_name", "")))
                if not random_sign:
                    continue
                q_idx = int(vrow.get("question_index", -1))
                alpha_abs = float(vrow.get("alpha_abs", float("nan")))
                layer_band = str(vrow.get("layer_band", ""))
                base_condition = str(vrow.get("base_condition", ""))
                rsub = random_rows[
                    (random_rows["question_index"].astype(int) == q_idx)
                    & (random_rows["base_condition"].astype(str) == base_condition)
                    & (random_rows["sign_name"].astype(str) == random_sign)
                    & (random_rows["layer_band"].astype(str) == layer_band)
                    & np.isclose(random_rows["alpha_abs"].astype(float), alpha_abs)
                ]
                if not len(rsub):
                    continue
                x_likeness = float(vrow.get("behavioral_target_likeness_cosine_0_to_1", float("nan")))
                r_likeness = pd.to_numeric(
                    rsub["behavioral_target_likeness_cosine_0_to_1"],
                    errors="coerce",
                ).astype(float)
                r_likeness = r_likeness[np.isfinite(r_likeness)]
                if not len(r_likeness) or not np.isfinite(x_likeness):
                    continue
                r_values = r_likeness.values.astype(float)
                random_mean = float(np.mean(r_values))
                random_p95 = float(np.percentile(r_values, 95))
                random_best = float(np.max(r_values))

                x_suppression = 1.0 - x_likeness
                r_suppression = 1.0 - r_values
                random_supp_mean = float(np.mean(r_suppression))
                random_supp_p95 = float(np.percentile(r_suppression, 95))
                random_supp_best = float(np.max(r_suppression))

                hard_random_rows.append(
                    {
                        "question_index": q_idx,
                        "base_condition": base_condition,
                        "sign_name": str(vrow.get("sign_name", "")),
                        "random_sign_name": random_sign,
                        "alpha_abs": alpha_abs,
                        "layer_band": layer_band,
                        "random_count": int(len(r_values)),
                        "x_target_likeness": x_likeness,
                        "random_target_likeness_mean": random_mean,
                        "random_target_likeness_p95": random_p95,
                        "random_target_likeness_best": random_best,
                        "lift_over_random_mean": x_likeness - random_mean,
                        "lift_over_random_p95": x_likeness - random_p95,
                        "lift_over_random_best": x_likeness - random_best,
                        "wins_over_random_mean": int(x_likeness > random_mean),
                        "wins_over_random_p95": int(x_likeness > random_p95),
                        "wins_over_random_best": int(x_likeness > random_best),
                        "x_suppression_from_target": x_suppression,
                        "random_suppression_mean": random_supp_mean,
                        "random_suppression_p95": random_supp_p95,
                        "random_suppression_best": random_supp_best,
                        "suppression_lift_over_random_mean": x_suppression - random_supp_mean,
                        "suppression_lift_over_random_p95": x_suppression - random_supp_p95,
                        "suppression_lift_over_random_best": x_suppression - random_supp_best,
                        "suppression_wins_over_random_mean": int(x_suppression > random_supp_mean),
                        "suppression_wins_over_random_p95": int(x_suppression > random_supp_p95),
                        "suppression_wins_over_random_best": int(x_suppression > random_supp_best),
                    }
                )

        behavioral_control_hard_random_df = pd.DataFrame(hard_random_rows)
        behavioral_control_hard_random_df.to_csv(
            RESULTS_DIR / "behavioral_control_axis_hard_random_comparison.csv",
            index=False,
        )
        if len(behavioral_control_hard_random_df):
            behavioral_control_hard_random_summary_df = (
                ...
                .agg(
                    mean_x_target_likeness=("x_target_likeness", "mean"),
                    mean_random_target_likeness=("random_target_likeness_mean", "mean"),
                    ...
                    win_rate_vs_random_mean=("wins_over_random_mean", "mean"),
                    win_rate_vs_random_p95=("wins_over_random_p95", "mean"),
                    win_rate_vs_random_best=("wins_over_random_best", "mean"),
                    mean_x_suppression=("x_suppression_from_target", "mean"),
                    mean_random_suppression=("random_suppression_mean", "mean"),
                    mean_suppression_lift_over_random_mean=("suppression_lift_over_random_mean", "mean"),
                    mean_suppression_lift_over_random_p95=("suppression_lift_over_random_p95", "mean"),
                    mean_suppression_lift_over_random_best=("suppression_lift_over_random_best", "mean"),
                    suppression_win_rate_vs_random_mean=("suppression_wins_over_random_mean", "mean"),
                    suppression_win_rate_vs_random_p95=("suppression_wins_over_random_p95", "mean"),
                    suppression_win_rate_vs_random_best=("suppression_wins_over_random_best", "mean"),
                    n_questions=("question_index", "nunique"),
            ...
            behavioral_control_hard_random_summary_df = pd.DataFrame()
        behavioral_control_hard_random_summary_df.to_csv(
            RESULTS_DIR / "behavioral_control_axis_hard_random_summary.csv",
            index=False,
        )
        ...
        if BEHAVIORAL_CONTROL_PRIMARY_ALPHA is not None:
            primary_eval_alpha = float(BEHAVIORAL_CONTROL_PRIMARY_ALPHA)
        else:
            primary_eval_alpha = float(max(BEHAVIORAL_CONTROL_ALPHA_VALUES)) if BEHAVIORAL_CONTROL_ALPHA_VALUES else 1.0
        primary_plus_likeness = summary_value(
            {
                "base_condition": REFERENCE_CONDITION,
                "intervention_kind": "vector_x",
                "sign_name": "plus_x",
                "alpha_abs": primary_eval_alpha,
        ...
        def hard_summary_value(filters: Dict[str, object], column: str) -> float:
        ...
        neutral_plus_win_mean = hard_summary_value(
            {
                "base_condition": REFERENCE_CONDITION,
                "sign_name": "plus_x",
                "alpha_abs": primary_eval_alpha,
        ...
        neutral_plus_win_p95 = hard_summary_value(
            {
                "base_condition": REFERENCE_CONDITION,
                "sign_name": "plus_x",
                "alpha_abs": primary_eval_alpha,
        ...
        neutral_plus_lift_p95 = hard_summary_value(
            {
                "base_condition": REFERENCE_CONDITION,
                "sign_name": "plus_x",
                "alpha_abs": primary_eval_alpha,
        ...
        neutral_plus_lift_best = hard_summary_value(
            {
                "base_condition": REFERENCE_CONDITION,
                "sign_name": "plus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "mean_lift_over_random_best",
        )
        target_minus_suppression_win_mean = hard_summary_value(
            {
                "base_condition": "target",
                "sign_name": "minus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "suppression_win_rate_vs_random_mean",
        )
        target_minus_suppression_win_p95 = hard_summary_value(
            {
                "base_condition": "target",
                "sign_name": "minus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "suppression_win_rate_vs_random_p95",
        )
        target_minus_suppression_lift_mean = hard_summary_value(
            {
                "base_condition": "target",
                "sign_name": "minus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "mean_suppression_lift_over_random_mean",
        )
        target_minus_suppression_lift_p95 = hard_summary_value(
            {
                "base_condition": "target",
                "sign_name": "minus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "mean_suppression_lift_over_random_p95",
        )
        target_minus_suppression_lift_best = hard_summary_value(
            {
                "base_condition": "target",
                "sign_name": "minus_x",
                "alpha_abs": primary_eval_alpha,
            ...
            "mean_suppression_lift_over_random_best",
        )
        behavioral_control_asymmetry_df = pd.DataFrame(
            [
                {
                    "primary_layer_band": primary_band,
                    "primary_eval_alpha": primary_eval_alpha,
                    "neutral_plus_x_lift_over_random_mean": plus_x_lift_over_random,
                    "neutral_plus_x_lift_over_random_p95": neutral_plus_lift_p95,
                    "neutral_plus_x_lift_over_best_random": neutral_plus_lift_best,
                    "neutral_plus_x_win_rate_vs_random_mean": neutral_plus_win_mean,
                    "neutral_plus_x_win_rate_vs_random_p95": neutral_plus_win_p95,
                    "target_minus_x_suppression_lift_over_random_mean": target_minus_suppression_lift_mean,
                    "target_minus_x_suppression_lift_over_random_p95": target_minus_suppression_lift_p95,
                    "target_minus_x_suppression_lift_over_best_random": target_minus_suppression_lift_best,
                    "target_minus_x_suppression_win_rate_vs_random_mean": target_minus_suppression_win_mean,
                    "target_minus_x_suppression_win_rate_vs_random_p95": target_minus_suppression_win_p95,
                    "asymmetry_label": (
                        "target_ablation_stronger_than_neutral_induction"
                        if np.isfinite(target_minus_suppression_win_mean)
                        and np.isfinite(neutral_plus_win_mean)
                        and target_minus_suppression_win_mean > neutral_plus_win_mean
                        else "neutral_induction_not_weaker_or_unknown"
                    ),
                }
            ]
        )
            ...
            layer_verdict_rows.append(
                {
                    "layer_band": sweep_band,
                    "primary_eval_alpha": primary_eval_alpha,
                    "primary_max_alpha": primary_eval_alpha,
                    ...
                    "visible_quality_ok": int((not np.isfinite(band_degenerate_rate)) or band_degenerate_rate <= 0.25),
                }
            )
            ...
            [
                {
                    ...
                    "visible_quality_ok": int(visible_quality_ok),
                    "neutral_plus_x_win_rate_vs_random_mean": neutral_plus_win_mean,
                    "neutral_plus_x_win_rate_vs_random_p95": neutral_plus_win_p95,
                    "neutral_plus_x_lift_over_random_p95": neutral_plus_lift_p95,
                    "neutral_plus_x_lift_over_best_random": neutral_plus_lift_best,
...
- Neutral +X target-likeness at alpha `{primary_eval_alpha:g}` / `{primary_band}`: `{primary_plus_likeness:.6g}`
- Neutral -X target-likeness at alpha `{primary_eval_alpha:g}` / `{primary_band}`: `{primary_minus_likeness:.6g}`
- Target -X target-likeness at alpha `{primary_eval_alpha:g}` / `{primary_band}`: `{target_minus_likeness:.6g}`
...
- Neutral +X win rate vs random mean: `{neutral_plus_win_mean:.6g}`
- Neutral +X win rate vs random p95: `{neutral_plus_win_p95:.6g}`
- Neutral +X lift over random p95: `{neutral_plus_lift_p95:.6g}`
- Target -X suppression: `{target_minus_suppression:.6g}`
- Target -X suppression lift over random mean: `{target_minus_suppression_lift_mean:.6g}`
- Target -X suppression win rate vs random mean: `{target_minus_suppression_win_mean:.6g}`
- Target -X suppression win rate vs random p95: `{target_minus_suppression_win_p95:.6g}`
...
- If `neutral +X` becomes more target-like than neutral baseline and random-vector baselines,
  `target -X` becomes less target-like than target baseline, and visible text does not degenerate,
...
def readiness_row(item: str, status: str, evidence: str, next_step: str) -> None:
...
readiness_row(
    "hidden_geometry",
    "supported" if verdict_label in {"geometry_shift_supported", "prompt_endpoint_geometry_supported_generation_mixed_or_not_run"} else "weak_or_failed",
    (
        f"target_projection={target_proj_mean:.6g}; "
        f"target_minus_best_control_projection={target_minus_best_control_projection:.6g}; "
        f"positive_fraction={target_pos_frac:.6g}"
    ),
    "Replicate across model families and non-self-reference text families.",
)
readiness_row(
    "paired_control_separation",
    "supported" if np.isfinite(worst_paired_projection_diff) and worst_paired_projection_diff > 0 and worst_paired_projection_win_fraction >= 0.8 else "weak_or_missing",
    (
        f"worst_paired_projection_diff={worst_paired_projection_diff:.6g}; "
        f"worst_win_fraction={worst_paired_projection_win_fraction:.6g}"
    ),
    "Increase neutral held-out question count and keep word/sentence/length controls.",
)
readiness_row(
    "generation_internal_axis",
    "supported" if len(behavioral_control_verdict_df) and float(behavioral_control_verdict_df.iloc[0].get("neutral_plus_x_generation_projection", np.nan)) > float(behavioral_control_verdict_df.iloc[0].get("random_plus_generation_projection", np.nan)) else "weak_or_not_run",
    behavioral_control_note.replace("\n", " ")[:500],
    "Report generation-state projection separately from visible-response transfer.",
)

visible_status = "not_run"
visible_evidence = "behavioral control-axis unavailable"
if len(behavioral_control_verdict_df):
    row = behavioral_control_verdict_df.iloc[0]
    visible_verdict = str(row.get("verdict", ""))
    visible_quality = float(row.get("visible_quality_ok", np.nan))
    lift_random = float(row.get("plus_x_lift_over_random", np.nan))
    deg_rate = float(row.get("neutral_plus_x_degenerate_response_rate", np.nan))
    win_mean = float(row.get("neutral_plus_x_win_rate_vs_random_mean", np.nan))
    win_p95 = float(row.get("neutral_plus_x_win_rate_vs_random_p95", np.nan))
    if visible_verdict == "behavioral_control_axis_supported":
        visible_status = "supported"
    elif visible_verdict == "partial_behavioral_control_axis_supported":
        visible_status = "partial"
    else:
        visible_status = "weak_or_failed"
    visible_evidence = (
        f"verdict={visible_verdict}; lift_over_random={lift_random:.6g}; "
        f"win_mean={win_mean:.6g}; win_p95={win_p95:.6g}; "
        f"degenerate_rate={deg_rate:.6g}; visible_quality_ok={visible_quality:.6g}"
    )
readiness_row(
    "visible_neutral_plus_x_readout",
    visible_status,
    visible_evidence,
    "For breakthrough language, neutral +X should beat random p95/best on held-out questions without degeneration.",
)

hard_random_status = "not_run"
hard_random_evidence = "hard random summary unavailable"
if len(behavioral_control_hard_random_summary_df):
    primary_hr = behavioral_control_hard_random_summary_df[
        (behavioral_control_hard_random_summary_df["base_condition"].astype(str) == REFERENCE_CONDITION)
        & (behavioral_control_hard_random_summary_df["sign_name"].astype(str) == "plus_x")
        & (behavioral_control_hard_random_summary_df["layer_band"].astype(str) == BEHAVIORAL_CONTROL_PRIMARY_LAYER_BAND)
        & np.isclose(behavioral_control_hard_random_summary_df["alpha_abs"].astype(float), float(BEHAVIORAL_CONTROL_PRIMARY_ALPHA or BEHAVIORAL_CONTROL_RANDOM_ALPHA))
    ]
    if len(primary_hr):
        best_row = primary_hr.iloc[0]
        hard_random_status = (
            "supported"
            if float(best_row.get("win_rate_vs_random_p95", 0.0)) >= 0.8
            and float(best_row.get("mean_lift_over_random_p95", 0.0)) > 0
            else "partial_or_weak"
        )
        hard_random_evidence = (
            f"lift_mean={float(best_row.get('mean_lift_over_random_mean', np.nan)):.6g}; "
            f"lift_p95={float(best_row.get('mean_lift_over_random_p95', np.nan)):.6g}; "
            f"lift_best={float(best_row.get('mean_lift_over_random_best', np.nan)):.6g}; "
            f"win_mean={float(best_row.get('win_rate_vs_random_mean', np.nan)):.6g}; "
            f"win_p95={float(best_row.get('win_rate_vs_random_p95', np.nan)):.6g}"
        )
readiness_row(
    "hard_random_specificity",
    hard_random_status,
    hard_random_evidence,
    "Use this as the main visible-readout gate; random mean alone is not enough.",
)

asymmetry_status = "not_run"
asymmetry_evidence = "asymmetry summary unavailable"
if len(behavioral_control_asymmetry_df):
    row = behavioral_control_asymmetry_df.iloc[0]
    asymmetry_status = "supported" if str(row.get("asymmetry_label", "")) == "target_ablation_stronger_than_neutral_induction" else "mixed"
    asymmetry_evidence = (
        f"label={row.get('asymmetry_label', '')}; "
        f"neutral_win_mean={float(row.get('neutral_plus_x_win_rate_vs_random_mean', np.nan)):.6g}; "
        f"target_supp_win_mean={float(row.get('target_minus_x_suppression_win_rate_vs_random_mean', np.nan)):.6g}"
    )
readiness_row(
    "ablation_vs_induction_asymmetry",
    asymmetry_status,
    asymmetry_evidence,
    "If target -X is consistently stronger than neutral +X, frame Vector X as an ablation/suppression axis first.",
)

readiness_row(
    "cross_model_replication",
    "supported" if REPLICATION_MODEL_COUNT >= 3 else "missing",
    f"replication_model_count={REPLICATION_MODEL_COUNT}",
    "Run the same locked neutral-question protocol on at least three model families/checkpoints.",
)
readiness_row(
    "cross_text_family_replication",
    "supported" if REPLICATION_TEXT_FAMILY_COUNT >= 3 else "missing",
    f"replication_text_family_count={REPLICATION_TEXT_FAMILY_COUNT}",
    "Run self-reference plus held-out non-self-reference induction families.",
)

supported_items = sum(1 for row in breakthrough_rows if row["status"] == "supported")
partial_items = sum(1 for row in breakthrough_rows if row["status"] in {"partial", "partial_or_weak", "mixed"})
if (
    REPLICATION_MODEL_COUNT >= 3
    and REPLICATION_TEXT_FAMILY_COUNT >= 3
    and hard_random_status == "supported"
    and visible_status in {"supported", "partial"}
):
    breakthrough_label = "breakthrough_ready_pending_external_review"
elif verdict_label == "geometry_shift_supported" and visible_status in {"supported", "partial"}:
    breakthrough_label = "strong_mechanistic_case_not_breakthrough_yet"
else:
    breakthrough_label = "preliminary_or_incomplete"
readiness_row(
    "overall_breakthrough_readiness",
    breakthrough_label,
    f"supported_items={supported_items}; partial_items={partial_items}",
    "Do not use breakthrough language until cross-model/family replication and hard visible-readout specificity are closed.",
)

breakthrough_readiness_df = pd.DataFrame(breakthrough_rows)
...
Overall label: `{breakthrough_label}`

This file separates strong mechanistic evidence from a defensible breakthrough claim.

```text
{breakthrough_readiness_df.to_string(index=False)}
```
""",
    )
...
This separates a strong mechanistic result from a defensible breakthrough claim.

```text
{breakthrough_readiness_df.to_string(index=False) if len(breakthrough_readiness_df) else "Breakthrough readiness audit unavailable."}
```
