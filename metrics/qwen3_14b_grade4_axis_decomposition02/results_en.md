...
```text
Source: metrics\qwen3_14b_breakthrough_grade_hardened\red_team_hidden_geometry_results_breakthrough_grade4_axis_decomposition02.zip
...
Layer bands in this archive: ['middle', 'late', 'all']
Alpha values: [0.1, 0.25, 0.5, 0.75]
Generation batch size: 16
```

Note: this completed archive used the broader legacy sweep `middle/late/all` with batch size 16. That does not invalidate the metrics; it adds the optional all-layer comparison. The cheaper future rerun default remains `middle/late`.
...
Mechanistically: the content component is large, but the order component is not numerical residue. In the middle band, `x_order_orth` keeps about `0.277756` of full-axis-scale energy and remains meaningfully aligned with the full axis.

The negative `cos(content, order)` matters: sentence-shuffled content and coherent-order residue are not two parallel pieces of one axis. They partially compensate each other, which is why `content_norm` can exceed `full_norm`.
...
This is a clean separator:

```text
sentence_shuffle strongly occupies x_content but almost not x_order_orth.
target strongly occupies x_full and x_order_orth.
```

That answers the Grade 4 question: coherent target ordering contributes a distinct direction beyond sentence-shuffled content.
...
`x_order_orth` ranks first by gap in middle/middle for both neutral and target. This is the central causal result.

## 6. Optional All-Layer Stress Test

This archive also ran `all`. For middle readout at alpha 0.75:
...
This confirms that the order component is not an artifact of a single middle hook. It is not the main localization claim, because global all-layer perturbation is coarser and less mechanistically clean than middle-only intervention.
...
This strengthens the conclusion that causal component controllability is localized to middle intervention, not late-only intervention.
